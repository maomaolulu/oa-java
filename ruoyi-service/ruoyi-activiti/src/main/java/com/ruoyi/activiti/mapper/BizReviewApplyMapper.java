 ...
 */
@Repository
public interface BizReviewApplyMapper extends BaseMapper<BizReviewApply>
{
    @Select(" select bu.id buId, bu.proc_inst_id, bu.title,bu.status,bu.result,d.dept_name,d2.dept_name companyName,d2.dept_id companyId,u.user_name,ra.id, " +
            "ra.create_time,ra.update_time,ra.remark,ra.title,ra.cc,ra.dept_id,ra.review_code,ra.types, " +
            ...
            "left join biz_review_apply ra on bu.table_id=ra.id " +
            ...
            " ${ew.customSqlSegment} ")
    List<BizReviewApplyDto> selectAll(@Param(Constants.WRAPPER) QueryWrapper wrapper);
}