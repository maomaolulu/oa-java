...
public class RedisAspect
{
    ...
    @Pointcut("@annotation(com.ruoyi.common.redis.annotation.RedisEvict2)")
    public void redisEvictPoint2()
    {
    }
    ...
    @After("redisEvictPoint2()")
    public void evictArray(JoinPoint point)
    {
        Method method = ((MethodSignature) point.getSignature()).getMethod();
        RedisEvict2 redisEvict = method.getAnnotation(RedisEvict2.class);
        // 获取RedisCache注解
        String fieldKey = parseKey(redisEvict.fieldKey(), method, point.getArgs());
        String rk = redisEvict.key() + ":" + fieldKey;
        logger.debug("<======切面清除rediskey:{} ======>" + rk);
    ...
    public Object WriteReadFromRedis(ProceedingJoinPoint point)
    {
        try
        {
            ...
            if (redisCache != null && redisCache.read())
            {
                ...
                logger.debug("参数:"+redisCache.fieldKey()+"---"+ method);
                if(point.getArgs()!=null){
                    Arrays.stream(point.getArgs()).forEach(arg-> logger.debug(arg.toString()));
                }
                ...
                if (obj == null)
                {
                    ...
                    // Redis 中存在，则刷新有效期
//                    redis.flush(rk,redisCache.expired());
                }
                return obj;
            }
        }
